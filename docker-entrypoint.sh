...
if [ "$1" == "neo4j" ]; then
    # Start graphileon interactor
    HOST=$(netstat -nr | grep '^0\.0\.0\.0' | awk '{print $2}') && sed -i -e 's/localhost/'"$HOST"'/g' /var/www/html/interactor/settings/js/install-controller.js && /usr/sbin/apache2ctl -D FOREGROUND &
